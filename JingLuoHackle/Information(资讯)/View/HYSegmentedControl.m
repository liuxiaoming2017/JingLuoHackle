...
@synthesize array4Btn=_array4Btn;
...
- (id)initWithOriginY:(CGFloat)y Titles:(NSArray *)titles delegate:(id)delegate
{
    ...
    if (self = [super initWithFrame:rect4View]) {
        ...
        for (int i = 0; i<[titles count]; i++) {
            ...
            [btn setTitleColor:[UtilityFunc colorWithHexString:@"#333333"]  forState:UIControlStateNormal];
            btn.titleLabel.font = [UIFont systemFontOfSize:14];
            [btn setTitleColor:[UtilityFunc colorWithHexString:@"#2fa1fb"] forState:UIControlStateSelected];
       ...
       UIView* _bottomLineImgView = [[UIView alloc] initWithFrame:CGRectMake(5.0f, HYSegmentedControl_Height-0.5, rect4View.size.width, 0.5)];
        _bottomLineImgView.backgroundColor=[UtilityFunc colorWithHexString:@"#d8d8d8"];
        [_scrollView addSubview:_bottomLineImgView];
        _bottomLineView = [[UIView alloc] initWithFrame:CGRectMake(5.0f, HYSegmentedControl_Height-1, width4btn-10.0f, 1.0f)];
        _bottomLineView.backgroundColor = [UtilityFunc colorWithHexString:@"#0088ff"] ;//UIColorFromRGBValue(0x454545);
