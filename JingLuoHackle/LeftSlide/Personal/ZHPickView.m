...
-(UIToolbar *)setToolbarStyle{
    ...
    UIBarButtonItem *lefttem=[[UIBarButtonItem alloc] initWithTitle:LocalString(@"取消") style:UIBarButtonItemStylePlain target:self action:@selector(remove)];
    
    UIBarButtonItem *centerSpace=[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemFlexibleSpace target:self action:nil];
    
    UIBarButtonItem *right=[[UIBarButtonItem alloc] initWithTitle:LocalString(@"确定") style:UIBarButtonItemStylePlain target:self action:@selector(doneClick)];
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        NSString *dateAndTime =  [dateFormatter stringFromDate:_datePicker.date];
        
        _resultString=[NSString stringWithFormat:@"%@",dateAndTime];
    }
