...
-(void)setFailBlock:(successBlock)S{
    if(S!=0){
        Block_release(self.reqFailBlock);
        
        self.reqFailBlock = Block_copy(S);
...
-(void)setSuccessBlock:(failBlock)R{
    if(R!=0){
        Block_release(self.reqSuccessBlock);
        
        self.reqSuccessBlock = Block_copy(R);
...
- (void)buildPostBody
{
	...
	if ([self postLength] > 0) {
		if ([requestMethod isEqualToString:@"POST"] || [requestMethod isEqualToString:@"DELETE"] || [requestMethod isEqualToString:@"HEAD"]) {
